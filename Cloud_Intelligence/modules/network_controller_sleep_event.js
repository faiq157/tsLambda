...
exports.handleUpdate = async function(pgWrite, payload) {
  if (payload) {
    // await pgWrite.connect();
    const checkEventLog = await pgWrite.query(
    ...
    if (checkEventLog.rows.length === 0) {
      const ncSleepEventLog = await pgWrite.query(cloudEventLogQuery, [
        "NC-SLEEP",
