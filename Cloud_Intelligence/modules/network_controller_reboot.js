...
exports.handleNCRebootUpdate = async function (payload) {
  ...
  let reset_sec = getSeconds("0D 00:00:00");
    ...
    let wakeUpTime = await getWakeUpTime(payload);
    ...
    if(checkWakeupInterval){
      ...
      if (isNccbUptimeReset && isLinuxUptimeReset && isAppUptimeReset) {
        ...
        await cloudEventLogResults(payload, "NC-REBOOT", "Network Controller Reboot", "firmware_update");
        ...
        await accuWeatherService.handler(nc_reboot_event);
      }
      ...
      if (!isNccbUptimeReset && !isLinuxUptimeReset && isAppUptimeReset) {
        ...
        await cloudEventLogResults(payload,"NC-APPLICATION-REBOOT","Network Controller Application Reboot", "firmware_update");
      }
...
const getWakeUpTime = async function(payload){
  let wakeUpTime = moment.utc();;
  //get nc wakeup time information
  const ncInfoRes = await exeQuery(db.ncInfoById,[payload.network_controller_id]);
...
const addWakeupEvent = async function(payload){
  console.log("WAKE");
  const checkWakeUpEvent = await exeQuery(
    `
  ...
  if (checkWakeUpEvent.rows.length === 0) {
    ...
    await queueHelper.queueDetectedAnomaly(info); // wakeup_event
  } else {
...
const cloudEventLogResults = async function (
  ...
  return await exeQuery(cloudEventLog, [
    event_name,
    20,
